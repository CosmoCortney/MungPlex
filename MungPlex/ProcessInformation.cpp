...
bool MungPlex::ProcessInformation::initMGBA()
{
	bool romFound = false;
	uint64_t bufLogo = 0;
	...
	for (const auto& region : GetRegionList())
	{
		...
		_process.ReadMemorySafe(&bufLogo, romBasePtr + 0x104, 8);

		if (bufLogo == 0x0B000DCC6666EDCE)
		{
			uint8_t typeFlag = 0;
			_process.ReadMemorySafe(&typeFlag, romBasePtr + 0x143, 1);

			if (typeFlag == 0x80 || typeFlag == 0xC0)
			{
				LoadSystemInformationJSON("GBC");
				setMiscProcessInfo("GameBoy Color", false, false, 2, 1);
				_platformID = GBC;
			}
			else
			{
				LoadSystemInformationJSON("GB");
				setMiscProcessInfo("GameBoy", false, false, 2, 1);
				_platformID = GB;
			}

			std::string title(16, '\0');
			_process.ReadMemorySafe(title.data(), romBasePtr + 0x134, 16);
			_gameName = title;

			uint16_t checksum = 0;
			_process.ReadMemorySafe(&checksum, romBasePtr + 0x14E, 2);
			_rpcGameID = _gameID = ToHexString(checksum, 4);

			uint32_t romSize = 0;
			_process.ReadMemorySafe(&romSize, romBasePtr + 0x148, 1);
			_systemRegions[0].BaseLocationProcess = romBasePtr;

			_systemRegions[0].Size = 0x8000 * (1 << romSize);
			romFound = true;
			break;
		}

		_process.ReadMemorySafe(&bufLogo, romBasePtr + 4, 8);

		if (bufLogo == 0x21A29A6951AEFF24)
		{
			setMiscProcessInfo("GameBoy Advance", false, false, 4, 4);
			LoadSystemInformationJSON("GBA");
			_platformID = GBA;
			_systemRegions[1].BaseLocationProcess = romBasePtr;

			std::string title(12, '\0');
			_process.ReadMemorySafe(title.data(), romBasePtr + 0xA0, 12);
			_gameName = title;

			std::string id(6, '\0');
			_process.ReadMemorySafe(id.data(), romBasePtr + 0xAC, 6);
			_rpcGameID = _gameID = id;

			_gameRegion = getRegionFromNintendoRegionCode(id[3]);
			_systemRegions[0].BaseLocationProcess = romBasePtr;
			romFound = true;
			break;
		}
	...
	if (_platformID == GB || _platformID == GBC)
	{
		for (const auto& region : GetRegionList())
		{
